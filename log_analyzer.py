    ...
    for ip, times in fails_by_ip.items():
        ...
        for end in range(len(times)):
            ...
            if count >= fail_threshold:
                # Severity classification (SOC-style)
                if count >= 5:
                    severity = "High"
                elif count >= 3:
                    severity = "Medium"
...
def write_report_md(report: Dict, out_path: str) -> None:
    ...
    if not report["alerts"]:
        ...
        for a in report["alerts"]:
            lines.append(f"### {a['alert_name']} ({a['severity']} Severity)\n")
            lines.append(f"- MITRE ATT&CK: {a['mitre_attack']['technique_id']} ({a['mitre_attack']['technique']})\n")
            lines.append(f"- Tactic: {a['mitre_attack']['tactic']}\n")
...
def main() -> int:
    ...
    if report["alerts"]:
        ...
        for a in report["alerts"]:
            print(
                f"  - {a['alert_name']} [{a['severity']}] from {a['ip']} "
                f"({a['failures_in_window']} fails/{a['window_minutes']}min, MITRE {a['mitre_attack']['technique_id']})"
            )
